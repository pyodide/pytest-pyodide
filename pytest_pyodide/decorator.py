...
class run_in_pyodide:
    ...
    def _code_template(self, args: tuple[Any, ...]) -> str:
        ...
        async def __tmp():
            ...
            \n{indent(self._get_code_prelude(), " " * 12)}
            ...
            \n{indent(self._get_code_epilogue(), " " * 12)}
            return result
...
def _get_coverage_path() -> Path:
    ...
    rand_part = "".join([choice("1234567890abcdef") for x in range(20)])
    return Path(f".coverage.emscripten.{_coverage_count}.{rand_part}")
